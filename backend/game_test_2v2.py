...
def print_game_state(game: BriscolaGame):
    ...
    for player in game.players:
        print(
            f"  {player.name} (Team {player.team}): Score = {player.score}, Hand size = {player.get_hand_size()}"
        )
...
def play_game():
    """Run a test game of 2v2 Briscola."""
    ...
    for player in game.players:
        ...
        print(f"  {player.name} (Team {player.team}): {player.score}")
    print(f"Team 1 total: {team_scores[1]}")
    print(f"Team 2 total: {team_scores[2]}")
    if winner:
        print(
            f"The winning team is Team {winner} with a score of {team_scores[winner]}."
        )
    else:
